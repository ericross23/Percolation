program label_main
    ...
    real,dimension(10000) :: disp, avg_disp
    integer,dimension(2) :: pos
    integer :: i, j, label, transv_ave, lon_ave, iprint
    ...
    do iter = 1, maxiter
      ...
      pos(1) = n_columns / 2
      pos(2) = n_rows / 2
      if(spanning_cluster(pos(1),pos(2)) == 0) go to 75
      do i = 1, 10000
        45 call random_number(randnum)
        if (randnum >= 0 .and. randnum < 0.25) then
          if (spanning_cluster(pos(1) + 1, pos(2)) == 1 .and. pos(1) + 1 <= n_columns) then
            pos(1) = pos(1) + 1
          else
            go to 45
          end if
        else if (randnum >= 0.25 .and. randnum < 0.50) then
          if (spanning_cluster(pos(1) - 1, pos(2)) == 1 .and. pos(1) - 1 > 0) then
            pos(1) = pos(1) - 1
          else
            go to 45
          end if
        else if (randnum >= 0.50 .and. randnum < 0.75) then
          if (spanning_cluster(pos(1), pos(2) + 1) == 1 .and. pos(2) + 1 <= n_rows) then
            pos(2) = pos(2) + 1
          else
            go to 45
          end if
        else
          if (spanning_cluster(pos(1), pos(2) - 1) == 1 .and. pos(2) - 1 > 0) then
            pos(2) = pos(2) - 1
          else
            go to 45
          end if
        end if
        disp(i) = sqrt((real(pos(1)) - n_columns/2) ** 2 + (real(pos(2)) - n_rows/2) ** 2)
        !print*, 'Displacement: ',disp(i), 'Position: ', pos(1), pos(2)
      end do
      do i = 1, 10000
        avg_disp(i) = avg_disp(i) + disp(i)
      end do
    end do
    avg_disp = avg_disp / maxiter
    p_avg = p_avg / maxiter
