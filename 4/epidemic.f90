program eden
  ...
  do loops = 1, cluster_size-1
    ...
    if((site(t_col+1,t_row) == 1 .or. site(t_col+1,t_row) == 2) &
 .and. (site(t_col-1,t_row) == 1 .or. site(t_col-1,t_row) == 2) &
 .and. (site(t_col,t_row+1) == 1 .or. site(t_col,t_row+1) == 2) &
 .and. (site(t_col,t_row-1) == 1 .or. site(t_col,t_row-1) == 2)) go to 100
    ...
    !print*, 'Column: ', t_col, 'Row: ', t_row
      ...
      if (randnum >= 0 .and. randnum < 0.25) then
        if (site(t_col + 1, t_row) == 0 .and. t_col <= n_columns) then
          call random_number(randnum)
          if (randnum < 0.55) then
            site(t_col + 1, t_row) = 1
          else
            site(t_col + 1, t_row) = 2
          end if
        else
          go to 110
        end if
    ...
    do i = 1, 100
      ...
      call random_number(randnum)
      t_row = ceiling(randnum*n_rows) + 1
      if(site(t_col,t_row) /= 1) go to 120
      if((site(t_col+1,t_row) == 1 .or. site(t_col+1,t_row) == 2) &
   .and. (site(t_col-1,t_row) == 1 .or. site(t_col-1,t_row) == 2) &
   .and. (site(t_col,t_row+1) == 1 .or. site(t_col,t_row+1) == 2) &
   .and. (site(t_col,t_row-1) == 1 .or. site(t_col,t_row-1) == 2)) go to 120
    ...
    do i = 1, n_columns
      do j = 1, n_rows
        write(50,'(I6)',advance='NO') site(i,j)
      end do
      write(50,*)
    end do
    close(50)
